...
src-tauri/
  ...
  tauri.{linux,windows,macos}.conf.json
                           externalBin, desktop-only by construction (ADR 0057) — Tauri
                           merges these in only for the platform each is named for, so
                           Android and iOS builds never see a sidecar they must not have
